...
def go(args):
    ...
    X_val[object_cols] = X_val[object_cols].fillna('').astype(str)  # Fill NaN first, then to str

    # Diagnostics (after definition; comment out if not debugging)
    logger.info(X_val.dtypes)
    logger.info(X_val[object_cols].apply(lambda col: col.apply(type).value_counts()))  # Fixed to per-column
    ...
    input_example[object_cols] = input_example[object_cols].fillna('').astype(str)
